  ...
  const handleLikeUpdate = async () => {
    ...
    try {
      const { error } = await supabaseDB
        .from('likes')
        .update({like: isClicked })
        .eq('id', param.id )
        .eq('users', user)

      error 
        ? alert('관심등록에서 문제가 발생했습니다.')
        : alert('관심등록이 완료됐습니다.')
    } catch (error) {
      console.error('Like update failed:', error)
      alert('관심등록 중 오류가 발생했습니다.')
    } finally{
